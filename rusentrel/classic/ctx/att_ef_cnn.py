...
def ctx_att_ef_cnn_custom_config(config):
    assert(isinstance(config, AttentionEndsAndFramesCNNConfig))
    config.modify_bags_per_minibatch(2)
    config.modify_weight_initializer(tf.contrib.layers.xavier_initializer())


def run_testing_att_ef_cnn(name_prefix=u'',
                           cv_count=1,
                           custom_config_func=ctx_att_ef_cnn_custom_config,
