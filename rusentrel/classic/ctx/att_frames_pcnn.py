...
def ctx_att_frames_pcnn_custom_config(config):
    assert(isinstance(config, AttentionFramesPCNNConfig))
    config.modify_bags_per_minibatch(2)
    config.modify_weight_initializer(tf.contrib.layers.xavier_initializer())


def run_testing_att_frames_pcnn(name_prefix=u'',
                                cv_count=1,
                                custom_config_func=ctx_att_frames_pcnn_custom_config,
